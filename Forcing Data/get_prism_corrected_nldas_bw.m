function get_prism_corrected_nldas_bw(Domain,StartMonth,StartYear,EndMonth,EndYear)
% Download and process PRISM-corrected NLDAS forcing data.  Output data
% will
% Usage: high_res_nldas(Domain,StartMonth,StartYear,EndMonth,EndYear)
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  UPDATED BY B.W. 03/30/2022 
%  --- bypass GRIB-->NETCDF conversions with updated data location
%  --- Fixed associated temperature conversion
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for TS = STStamp:ETStamp
    ...
    if dvec(3) == 1
        ...
        if d == 1
            ...
            hr = '00';
            url = [NLDASDataLoc '/' yr '/' doy_str '/NLDAS_FORA0125_H.A' yr mo dy '.' hr '00.020.nc'];
            of = [tmp_dir '/' hr '.nc'];
            % Download NLDAS data
            ...
            for hour = 0:23
                ...
                for try_iter = 1:5
                    try
                        ...
                        AIRT = ncread([tmp_dir '/' hr '.nc'],'Tair');        airt(:,:,c) = AIRT(locx,locy);
                        SPFH = ncread([tmp_dir '/' hr '.nc'],'Qair');        spfh(:,:,c) = SPFH(locx,locy);
                        PRES = ncread([tmp_dir '/' hr '.nc'],'PSurf');       pres(:,:,c) = PRES(locx,locy);
                        UWIND = ncread([tmp_dir '/' hr '.nc'],'Wind_E');     uwind(:,:,c) = UWIND(locx,locy);
                        VWIND = ncread([tmp_dir '/' hr '.nc'],'Wind_N');     vwind(:,:,c) = VWIND(locx,locy);
                        DLWRF = ncread([tmp_dir '/' hr '.nc'],'LWdown');     dlwrf(:,:,c) = DLWRF(locx,locy);
                        APCP = ncread([tmp_dir '/' hr '.nc'],'Rainf');       apcp(:,:,c) = APCP(locx,locy);
                        DSWRF = ncread([tmp_dir '/' hr '.nc'],'SWdown');     dswrf(:,:,c) = DSWRF(locx,locy);
        ...
        tmean_bias = max(-10,min(10,mean((airt_max+airt_min)/2,3) - (tmean+273.5)));
        tmean_bias(isnan((tmean+273.5))) = NaN;
        ...
        for dim = 1:DIM
            ...
            for hour = 0:23 
                ...
                % add lat/lon blocks above var loop
                nccreate(ncfile,'lat','Dimensions', {'lat', sz(2)},'Datatype','single');
                ncwriteatt(ncfile,'/lat','units','degrees_north');
                ncwrite(ncfile,'lat',lat(locy));
